...
def register(request):
    # A boolean vaue for telling the template
    # whether the registration was succesful.
    # Set to False initially. Code changes value to
    # True when registration succeeds.
    registered = False

    # If it's a HTTP POST (i.e. get input from user), we're
    # interested in processing form data. (As form exists for getting
    # input from the user.)
    if request.method == 'POST':
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            # Since we need to set the user attribute (=field) ourselves,
            ...
            # If so, we need to get that profile picture from
        ...
        # Not a HTTP POST (i.e. request.method != 'POST'),
        # so we render our form using two ModelForm instances.
...
def user_login(request):
    ...
    if request.method == 'POST':
        ...
        if user:
            # Is the account active? The account could have been disabled.
            if user.is_active:
    ...
    # This scenario would most likely be a HTTP GET.
        ...
        # blank dictionary object... (which is same as putting {})
        return render(request, 'rango/login.html')

# Restricting access with a decorator (the decorator is @login_required)
...
# those that are logged in can access this view.
# Decorators must be Before function!
@login_required
